...
DEF_OUT_DIR="../output"
OUT_DIR="../../results/${MODEL}/nodup/0.66"
if [ ! -d "${OUT_DIR}" ] ; then
...
for j in astride delta0 uniform normal ; do
   ...
   for k in total block event ; do
      for i in 0 m d s x ; do
            ...
            echo "dataset = history_scram-${i}f  strategy = ${k}  procedure = ${j}  trim = 0.66  nodup = rows"
            if [ ! -d "${RES_DIR}" ] ; then
            ...
            echo "     -m ${MODEL} -s ${k} -p -e -d 1" \
                  >> ${RES_DIR}/run_history_scram-${i}f_${k}_${j}.log         
            ./trim -v1 -i history_scram-${i}f.csv -t 'updated' -c 0.66 -m ${MODEL} \
                    -s ${k} -p -e -d 1 >> ${RES_DIR}/run_history_scram-${i}f_${k}_${j}.log
