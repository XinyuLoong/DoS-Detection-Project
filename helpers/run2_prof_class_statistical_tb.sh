...
for j in astride delta0 uniform normal ; do
   ...
   for k in total block ; do
      for i in 0 m d s x ; do
            ...
            echo "dataset = history_scram-${i}f  strategy = ${k}  procedure = ${j}  trim = 0.5"
            if [ ! -d "${RES_DIR}" ] ; then
               mkdir -p ${RES_DIR} || exit 1
            fi

            echo "trim -v1 -i history_scram-${i}f.csv -t 'updated' -c 0.5" \
                  > ${RES_DIR}/run_history_scram-${i}f_${k}_${j}.log
            echo "     -m ${MODEL} -s ${k} -p -e" \
                  >> ${RES_DIR}/run_history_scram-${i}f_${k}_${j}.log         
            ./trim -v1 -i history_scram-${i}f.csv -t 'updated' -c 0.5 -m ${MODEL} \
