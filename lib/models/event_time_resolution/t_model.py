...
def model_trim(df, event_column_list, timestamp_column, fake_timestamp=False,
   ...
   mlib.vprint(mlib.m_config["cli"]["trim_verb"], 
               f"Event Time Resolution Model: trims by timestamp", 1)
   ...
   # this model can trim only duplicate events which occur more than 
   # mlib.m_config["trimming"]["max_dup"] number of times
   max_dup = mlib.m_config["trimming"]["max_dup"]
   max_rows_totrim = len(df) - len(df.groupby(event_column_list).head(max_dup))
   if max_rows_totrim < rows_totrim:
      print((f"The Event Time Resolution Model can efficiently trim only "
             f"{max_rows_totrim} events, but {rows_totrim} requested. Exiting."),
            file=sys.stderr)
      sys.exit(1)
   else:
      mlib.vprint(mlib.m_config["cli"]["trim_verb"], (f"Trimming {rows_totrim} "
                  f"events out of {max_rows_totrim} possible"), 1)
   ...
   mlib.vprint(mlib.m_config["cli"]["trim_verb"], 
               f"Event Time Resolution Model: initial information = {total_info}", 1)
   ...
   if mlib.m_config['cli']['save_evolution']:
      mlib.vprint(mlib.m_config["cli"]["trim_verb"], 
               (f"Event Time Resolution Model: Saving evolution = "
      ...
      dtypes = {
          ...
          'TotalTrimmedEvents': pd.Series(dtype=int),
   ...
   # final touchups
   mlib.vprint(mlib.m_config["cli"]["trim_verb"], 
                 f"Event Time Resolution Model: information after trimming = {total_info}", 1)
   ...
   if mlib.m_config['cli']['save_evolution']:
      ...
      # find the order of computation by number of trimmed rows
      mlib.n_trimmed_order(info_evol_df)
